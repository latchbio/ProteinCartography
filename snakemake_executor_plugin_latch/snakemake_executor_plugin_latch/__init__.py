...
class Executor(RemoteExecutor):
    ...
    def run_job(self, job: JobExecutorInterface):
        ...
        # remove the python -m ... prefix
        command = self.format_job_exec(job).split()[2:]
        command[0] = "snakemake"
        ...
        if image is None:
            raise ValueError(
                f"{rule} - {job.jobid}: rule must have a container image configured to"
                " run on latch"
            )
        ...
        if gpus > 0 and gpu_type is None:
            raise ValueError(
                f"{rule} - {job.jobid}: rule that requests gpus must also specify a gpu"
                " type"
            )
